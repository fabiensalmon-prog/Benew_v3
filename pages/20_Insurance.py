...
with st.form("ins_quiz"):
    employed = st.radio("Êtes-vous salarié(e) en France ?", [t("yes"),t("no")], horizontal=True)
    family = st.radio("Couverture pour la famille ?", [t("yes"),t("no")], horizontal=True)
    car = st.radio("Avez-vous une voiture ?", [t("yes"),t("no")], horizontal=True)
    tenant = st.radio("Louez-vous un logement ?", [t("yes"),t("no")], horizontal=True)
    submitted = st.form_submit_button(t("see_rec"))
if submitted:
    out = []
    if tenant == t("yes"): out.append("**Habitation** (obligatoire pour locataires).")
    if car == t("yes"): out.append("**Auto** (obligatoire).")
    if employed == t("yes"):
        out.append("**Mutuelle d’entreprise** (souvent obligatoire, cofinancée).")
    else:
        out.append("**Mutuelle individuelle** conseillée (lunettes/dentaire/hospitalisation).")
    if family == t("yes"): out.append("Contrat **famille** (meilleure couverture globale).")
    st.success(" • ".join(out))
    st.info("Repère budget **mutuelle** (indicatif) : ~30–60€/mois (seul) ; ~60–120€/mois (famille). Comparez garanties & délais de carence.")
